...
{
    printf("ROOT MODULE\n");
    printf("ROOT MODULE DATA: %dx ADD, %dx SUB, %dx MUL, %dx DIV, %dx MOD.\n",
        MODULE_SUMADD(node), MODULE_SUMSUB(node), MODULE_SUMMUL(node), MODULE_SUMDIV(node), MODULE_SUMMOD(node));
    TRAVstmts(node);
    printf("END ROOT MODULE\n");
    return node;
}
